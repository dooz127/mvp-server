...
io.on('connection', (socket) => {
    ...
    socket.on('host-join', (data) => {
        // Check to see if id passed in url corresponds to id of a hunt in database
        MongoClient.connect(url, function (err, db) {
            if (err) {
                throw err;
            }

            var dbo = db.db('scavengedb');
            var query = { id: parseInt(data.id) };
            dbo.collection('hunts')
                .find(query)
                .toArray(function (err, result) {
                    ...
                    // A hunt was found with the id passed in url
            ...
            MongoClient.connect(url, function (err, db) {
                ...
                dbo.collection('hunts')
                    ...
                    .toArray(function (err, res) {
                        ...
                        var item = res[0].items[0].item;

                        socket.emit('gameItems', {
                            item1: item,
    ...
    socket.on('playerSubmission', function (num) {
        var player = players.getPlayer(socket.id);
        var hostId = player.hostId;
        var playerNum = players.getPlayers(hostId);
        var game = games.getGame(hostId);
        if (game.gameData.itemLive == true) {
            // if the item is still live
            player.gameData.answer = num;
            game.gameData.playersAnswered += 1;

            var gameItem = game.gameData.item;
            var gameid = game.gameData.gameid;
            ...
            MongoClient.connect(url, function (err, db) {
                ...
                dbo.collection('hunts')
                    ...
                    .toArray(function (err, res) {
                        ...
                        // TO DO: interface with Amazon Rekognition to determine if submission correct
                        ...
                        if (
                            game.gameData.playersSubmitted === playerNum.length
                        ) {
                            game.gameData.itemLive = false; // Item has ended bc players all submitted under time
                            var playerData = players.getPlayers(game.hostId);
                            io.to(game.pin).emit(
                                'itemOver',
                                playerData,
                                correctAnswer
                            ); // Tell everyone that item is over
                        } else {
                            // update host screen of num players submitted
                            io.to(game.pin).emit('updatePlayersSubmitted', {
                                playersInGame: playerNum.length,
                                playersSubmitted: game.gameData.playersSubmitted
                            });
                        }
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            dbo.collection('hunts')
                ...
                .toArray(function (err, res) {
                    ...
                    var correctAnswer =
                        res[0].questions[gameQuestion - 1].correct;
                    io.to(game.pin).emit('itemOver', playerData, correctAnswer);
                    db.close();
                });
        });
    });
    ...
    socket.on('nextItem', function () {
        ...
        MongoClient.connect(url, function (err, db) {
            ...
            dbo.collection('hunts')
                ...
                .toArray(function (err, res) {
                    ...
                    if (res[0].items.length >= game.gameData.item) {
                        var itemNum = game.gameData.item - 1;
                        var item = res[0].items[itemNum].item;
        ...
        io.to(game.pin).emit('nextHuntPlayer');
    });
    ...
    socket.on('newHunt', function (data) {
        MongoClient.connect(url, function (err, db) {
            if (err) {
                throw err;
            }
            ...
            dbo.collection('hunts')
                ...
                .toArray(function (err, result) {
                    ...
                    var game = data;
                    dbo.collection('hunts').insertOne(
                        game,
                        function (err, res) {
                            if (err) {
                                throw err;
