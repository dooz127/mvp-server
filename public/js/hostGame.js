...
socket.on('gameItems', function (data) {
    document.getElementById('item').innerHTML = data.item1;
    document.getElementById('playersSubmitted').innerHTML =
        'Players Submitted 0 / ' + data.playersInGame;
    updateTimer();
});

socket.on('updatePlayersSubmitted', function (data) {
    document.getElementById('playersSubmitted').innerHTML =
        'Players Submitted ' +
        data.playersSubmitted +
        ' / ' +
        data.playersInGame;
});

socket.on('itemOver', function (playerData, correct) {
    clearInterval(timer);
    var answer1 = 0;
    var answer2 = 0;
    var answer3 = 0;
    ...
    document.getElementById('playersSubmitted').style.display = 'none';
...
function nextItem() {
    document.getElementById('nextItemButton').style.display = 'none';
    ...
    document.getElementById('playersSubmitted').style.display = 'block';
    document.getElementById('timerText').style.display = 'block';
    document.getElementById('num').innerHTML = ' 30';
    socket.emit('nextItem'); // Tell server to start new item
}
...
socket.on('GameOver', function (data) {
    ...
    document.getElementById('item').innerHTML = 'GAME OVER';
    document.getElementById('playersSubmitted').innerHTML = '';
